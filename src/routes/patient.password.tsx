...
export const Route = createFileRoute("/patient/password")({ component: PatientPassword });
...
function PatientPassword() {
  ...
  return (
    <Page>
      <StepBar states={["active", "inactive", "inactive"]} />
      <H>Create your password</H>
      <Body>Choose a strong password to secure your Haibu Diabetes account.</Body>
      ...
      {filled ? (
        <PrimaryButton to="/patient/verify-method">Continue</PrimaryButton>
      ) : (
        <PrimaryButton disabled>Continue</PrimaryButton>
      )}

      <TestingLink to="/patient/verify-method">skip to next screen →</TestingLink>
      <BackLink to="/" />
    </Page>
  );
}
